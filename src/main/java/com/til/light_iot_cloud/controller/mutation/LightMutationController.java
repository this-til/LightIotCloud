...
import com.til.light_iot_cloud.enums.DisplayMode;
...
public class LightMutationController {
    ...
    public Result<Void> reportUpdate(Device light, @Argument LightData lightDataInput) {
        ...
        if (!(deviceContext instanceof DeviceContext.LightContext)) {
        ...
        boolean save = lightDataService.save(lightDataInput);
        return Result.ofBool(save);
    }
     ...
     */
    @SchemaMapping(typeName = "LightMutation")
    public Result<Void> switchDisplayMode(Device light, @Argument DisplayMode displayMode) {
        return commandDown(light, LightCommandKey.DISPLAY_MODE, displayMode.getValue());
    }
}
